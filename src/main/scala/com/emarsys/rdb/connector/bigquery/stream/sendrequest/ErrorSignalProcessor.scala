...
object ErrorSignalProcessor {
  def apply()(implicit materializer: Materializer): Graph[FlowShape[HttpResponse, Boolean], NotUsed] = GraphDSL.create() { implicit builder =>
    ...
    val errorMapper = builder.add(Flow[HttpResponse].map(response => response.status match {
      ...
      case otherStatus                            => throw new IllegalStateException(s"Unexpected error in response: $otherStatus, ${getErrorBody(response)}")
    }))
  ...
  private def getErrorBody(response: HttpResponse)(implicit materializer: Materializer) = {
    implicit val ec: ExecutionContext = materializer.executionContext
    import concurrent.duration._
    //TODO this await is bad
    Await.result(response.entity.convertToString(), 3.seconds)
  }
}
