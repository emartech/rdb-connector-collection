...
import java.sql.{Blob, CallableStatement, Clob, Connection, DatabaseMetaData, NClob, PreparedStatement, SQLWarning, SQLXML, Savepoint, ShardingKey, Statement, Struct}
import java.util
import java.sql
import java.util.Properties
import java.util.concurrent.Executor

class RouterConnection(val delegate: Connection) extends Connection {
  override def createStatement(): Statement = new RouterStatement(delegate.createStatement())

  override def prepareStatement(sql: String): PreparedStatement = new RouterPreparedStatement(delegate.prepareStatement(sql), sql)
  ...
  override def createStatement(resultSetType: Int, resultSetConcurrency: Int): Statement = new RouterStatement(delegate.createStatement(resultSetType, resultSetConcurrency))

  override def prepareStatement(sql: String, resultSetType: Int, resultSetConcurrency: Int): PreparedStatement = new RouterPreparedStatement(delegate.prepareStatement(sql, resultSetType, resultSetConcurrency), sql)
  ...
  override def createStatement(resultSetType: Int, resultSetConcurrency: Int, resultSetHoldability: Int): Statement = new RouterStatement(delegate.createStatement(resultSetType, resultSetConcurrency, resultSetHoldability))

  override def prepareStatement(sql: String, resultSetType: Int, resultSetConcurrency: Int, resultSetHoldability: Int): PreparedStatement = new RouterPreparedStatement(delegate.prepareStatement(sql, resultSetType, resultSetConcurrency, resultSetHoldability), sql)
  ...
  override def prepareStatement(sql: String, autoGeneratedKeys: Int): PreparedStatement = new RouterPreparedStatement(delegate.prepareStatement(sql, autoGeneratedKeys), sql)

  override def prepareStatement(sql: String, columnIndexes: Array[Int]): PreparedStatement = new RouterPreparedStatement(delegate.prepareStatement(sql, columnIndexes), sql)

  override def prepareStatement(sql: String, columnNames: Array[String]): PreparedStatement = new RouterPreparedStatement(delegate.prepareStatement(sql, columnNames), sql)
  ...
  override def beginRequest(): Unit = delegate.beginRequest()

  override def endRequest(): Unit = delegate.endRequest()

  override def setShardingKeyIfValid(shardingKey: ShardingKey, superShardingKey: ShardingKey, timeout: Int): Boolean = delegate.setShardingKeyIfValid(shardingKey, superShardingKey, timeout)

  override def setShardingKeyIfValid(shardingKey: ShardingKey, timeout: Int): Boolean = delegate.setShardingKeyIfValid(shardingKey, timeout)

  override def setShardingKey(shardingKey: ShardingKey, superShardingKey: ShardingKey): Unit = delegate.setShardingKey(shardingKey, superShardingKey)

  override def setShardingKey(shardingKey: ShardingKey): Unit = delegate.setShardingKey(shardingKey)
