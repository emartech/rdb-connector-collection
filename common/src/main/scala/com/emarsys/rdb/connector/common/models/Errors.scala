...
object Errors {
  ...
  case class ErrorPayload(errorCategory: ErrorCategory, error: String, message: String, causes: List[Cause], context: Option[Context]) extends ConnectorError(message) {
  ...
  object ErrorPayload {
    def apply(errorCategory: ErrorCategory, error: ErrorName, message: String, causes: List[Cause], context: Option[Context]): ErrorPayload =
