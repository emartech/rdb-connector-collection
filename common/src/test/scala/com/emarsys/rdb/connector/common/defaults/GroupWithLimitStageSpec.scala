...
class GroupWithLimitStageSpec extends AnyWordSpecLike with Matchers with BeforeAndAfterAll {
  ...
  "GroupWithLimitStage" should {
    ...
    "limit elements from the same group" in {
      val result =
        Await.result(Source(header :: data1).via(GroupWithLimitStage(Seq("id", "name"), 2)).runWith(Sink.seq), timeout)
    ...
    "limit elements from multiple groups" in {
      val result = Await.result(
        Source(header :: data1 ++ data2).via(GroupWithLimitStage(Seq("id", "name"), 2)).runWith(Sink.seq),
        timeout
      )
    ...
    "limit elements from multiple groups shuffled" in {
      ...
      val result = Await.result(
        Source(header :: shuffled).via(GroupWithLimitStage(Seq("id", "name"), 2)).runWith(Sink.seq),
        timeout
      )
    ...
    "emits instantly" in {
      val (sourceProbe, sinkProbe) = TestSource
        .probe[Seq[String]]
        .via(GroupWithLimitStage(Seq("id", "name"), 2))
    ...
    "can handle empty results" in {
      val result =
        Await.result(Source(List()).via(GroupWithLimitStage(Seq("id", "name"), 2)).runWith(Sink.seq), timeout)
    ...
    "can handle oneliner results" in {
      val result =
        Await.result(Source(header :: Nil).via(GroupWithLimitStage(Seq("id", "name"), 2)).runWith(Sink.seq), timeout)
    ...
    "can handle lowercase refs with uppercase headers" in {
      ...
      val result = Await.result(
        Source(upperHeader :: data1 ++ data2.take(2)).via(GroupWithLimitStage(Seq("id", "name"), 2)).runWith(Sink.seq),
        timeout
      )
