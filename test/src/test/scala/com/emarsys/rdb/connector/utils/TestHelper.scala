...
object TestHelper {
  ...
  def selectUniqueValueMock(select: SimpleSelect, connector: Connector, timeout: FiniteDuration)(
  ...
  def selectExactNumberMock(number: Int, tableName: String, connector: Connector, timeout: FiniteDuration)(
      implicit ec: ExecutionContext
  ) = {
    when(
  ...
  def emptyResultSelectMock(select: SimpleSelect, connector: Connector, timeout: FiniteDuration)(
      implicit ec: ExecutionContext
  ) = {
    when(connector.simpleSelect(select, timeout)).thenReturn(Future(Right(Source(List.empty))))
  }
}
