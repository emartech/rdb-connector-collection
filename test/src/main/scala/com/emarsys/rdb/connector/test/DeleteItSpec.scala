...
trait DeleteItSpec extends WordSpecLike with Matchers with BeforeAndAfterEach with BeforeAndAfterAll {
  ...
  val simpleSelect = SimpleSelect(
    ...
    TableName(tableName),
    where = Some(
      And(Seq(EqualToValue(FieldName("A2"), Value("2")), EqualToValue(FieldName("A3"), Value("1"))))
    )
  )

  val complexSelect = SimpleSelect(
    AllField,
    TableName(tableName),
    where = Some(
      Or(
  ...
  val simpleNullSelect = SimpleSelect(AllField, TableName(tableName), where = Some(IsNull(FieldName("A2"))))
  ...
  s"DeleteSpec $uuid" when {
    ...
    "#delete" should {
      ...
      "delete matching rows by simple criterions" in {
        ...
        Await
          .result(connector.simpleSelect(simpleSelectAllWithExpectedResultSize(7), queryTimeout), awaitTimeout)
          .map(stream => Await.result(stream.runWith(Sink.seq), awaitTimeout).size) shouldBe Right(7)
        Await
      ...
      "delete matching rows by complex criterions" in {
        ...
        Await
          .result(connector.simpleSelect(simpleSelectAllWithExpectedResultSize(6), queryTimeout), awaitTimeout)
          .map(stream => Await.result(stream.runWith(Sink.seq), awaitTimeout).size) shouldBe Right(6)
        Await
          .result(connector.simpleSelect(complexSelect, queryTimeout), awaitTimeout)
          .map(stream => Await.result(stream.runWith(Sink.seq), awaitTimeout).size) shouldBe Right(0)
      }
      ...
      "accept null values" in {
        ...
        Await
          ...
          .map(stream => Await.result(stream.runWith(Sink.seq), awaitTimeout).size) shouldBe Right(6)
        Await
          .result(connector.simpleSelect(simpleNullSelect, queryTimeout), awaitTimeout)
          .map(stream => Await.result(stream.runWith(Sink.seq), awaitTimeout).size) shouldBe Right(0)
      }
    }
  }
